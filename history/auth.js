...
function setToken(t) { localStorage.setItem('site_token', t); }
function clearToken() { localStorage.removeItem('site_token'); localStorage.removeItem('site_user'); }
...
function showWelcomeMessage(username, role, isDev) {
  ...
  if (isDev) {
    ...
    const roleMap = {
      'Chairperson': '主任委员', 'ExecutiveDeputyChair': '常务副主任委员',
      'SupervisorGeneral': '总监制委员', 'DeputySupervisor': '总副监制委员',
      'Reviewer': '审定委员', 'DraftWriter': '执笔委员',
      'MaterialCollector': '执书委员', 'user': ''
    };
    ...
    if (role && role !== 'user') {
      ...
      adminLink.style.cssText = 'display:inline-block;margin-left:12px;padding:4px 12px;background:rgba(99,102,241,0.3);border:1px solid rgba(99,102,241,0.6);border-radius:6px;color:#c7d2fe;font-size:13px;text-decoration:none;transition:all 0.2s';
      adminLink.onmouseenter = function() { this.style.background = 'rgba(99,102,241,0.5)'; };
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function verifyAndEnter() {
    ...
    if (!username) { errorDiv.textContent = '请填写姓名'; return; }
    if (!password) { errorDiv.textContent = '请填写密码'; return; }
